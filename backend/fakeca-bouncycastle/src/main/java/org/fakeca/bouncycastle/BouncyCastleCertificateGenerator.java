...
public class BouncyCastleCertificateGenerator implements CertificateGenerator {
    ...
    public Certificate generate(CertificateSpec spec, Certificate authorityCertificate) {
        ...
        try {
            ContentSigner signer = new JcaContentSignerBuilder("SHA256WithRSA").build(authorityCertificate.getKeyPair().getPrivate());
    ...
    private Certificate makeCertificate(CertificateSpec spec,X500Name issuerName, KeyPair keyPair, ContentSigner contentSigner){
        ...
        try {
            ...
            return new Certificate(spec, String.valueOf(holder.getSerialNumber()), keyPair,
                    holder.getEncoded());
    ...
    private KeyUsage makeKeyUsage(CertificateSpec spec){
        int keyUsageInt = 0;
        for(PublicKeyUsage keyUsage: spec.getPublicKeyUsages()){
            switch (keyUsage){
                case DigitalSignature -> keyUsageInt += KeyUsage.digitalSignature;
                case KeyCertificateSign -> keyUsageInt += KeyUsage.keyCertSign;
                case KeyEncipherment -> keyUsageInt += KeyUsage.keyEncipherment;
                case NonRepudiation -> keyUsageInt += KeyUsage.nonRepudiation;
                case SignCRL -> keyUsageInt += KeyUsage.cRLSign;
            }
        }
        return new KeyUsage(keyUsageInt);
    }
