...
public class Certificate {
    ...
    public Certificate(String serial, CertificateSubject subject, LocalDate expiryDate,  KeyStrength keyStrength, Set<PublicKeyUsage> keyUsages, KeyPair keyPair, byte[] derBytes){
    ...
    private void figureType(){
        if(subject instanceof EmployeeIdentitySubject)
            ...
            this.type = CertificateType.TLS_SSL;
    ...
    public Certificate(CertificateSpec spec, String serial, KeyPair keyPair, byte[] derBytes){
        if(spec==null)
    ...
    public Set<PublicKeyUsage> getKeyUsages() {
        return keyUsages;
    }
    ...
    public boolean exportDER(File file){
        ...
        try {
            ...
            throw new FakeCAException("failed to create file: "+file.getAbsolutePath(), ex);
        }
        try(FileOutputStream stream = new FileOutputStream(file)){
            this.exportDER(stream);
        }catch(IOException ex){
            throw new FakeCAException("error creating file stream: "+file.getAbsolutePath(),
                    ex);
        }
        return true;
    }
    ...
    public void exportDER(OutputStream stream){
        ...
        try {
            ...
            throw new FakeCAException("error writing encoded certificate to stream", ex);
        }
    }
