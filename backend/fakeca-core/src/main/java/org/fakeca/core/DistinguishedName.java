 ...
 * A distinguished commonName is specified as a string consisting of a sequence of attribute type/value pairs separated by a semicolon (';' U+003B). The general format is
 *
...
public record DistinguishedName(Set<RelativeDistinguishedName> relativeDistinguishedNames) {
    ...
    public DistinguishedName(Set<RelativeDistinguishedName> relativeDistinguishedNames){
        ...
        if(relativeDistinguishedNames.stream().filter(it -> it.type() == SubjectAttributeType.CommonName)
                .findAny().isEmpty())
            throw new IllegalArgumentException("Missing RDN with " + SubjectAttributeType.CountryName.name()+ " attribute type");
